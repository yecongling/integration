...
(
    ...
    `options`             json                                                                   DEFAULT NULL COMMENT 'endpoint的特定的配置选项',
    `headers`             json                                                                   DEFAULT NULL COMMENT 'HTTP或JMS等协议的请求头信息',
    `timeout`             int                                                           NOT NULL DEFAULT '0' COMMENT '连接或操作的超时时间',
    `retry_policy`        json                                                                   DEFAULT NULL COMMENT '重试策略配置，如重试次数、重试间隔等',
    ...
    KEY `idx_endpoint_type` (`type`) USING BTREE,
